...
public class UUFinshFragment extends Fragment implements
	...
	private RelativeLayout load_date_frag2;
	private SpotsDialog loadingDialog;
	private String port;
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
			Bundle savedInstanceState) {
		// TODO Auto-generated method stub
		view = inflater.inflate(R.layout.activity_uuorderone, container, false);// 已完成
		return view;
	}
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		list_view = (ListView) view.findViewById(R.id.order_auto_listview);
		load_date_frag2 = (RelativeLayout) view.findViewById(R.id.load_date_frag2);
		noOrderRel = (LinearLayout) view
				.findViewById(R.id.order_list_no_data_end_rel);
		noOrderBtn = (Button) view
				.findViewById(R.id.order_list_no_data_end_btn);
		noOrderTextOne = (TextView) view
				.findViewById(R.id.order_list_no_data_end_text_one);
		noOrderTextTwo = (TextView) view
				.findViewById(R.id.order_list_no_data_end_text_two);
		mSwipeLayout = (SwipeRefreshLayout) view
				.findViewById(R.id.order_finish_swipe_container);
	...
	public void onAttach(Activity activity) {
		...
		mActivity.setFinshHandler(mHandler);
	}
	...
	private void loadData(final int what,final boolean msg) {
		...
		params.add("orderStatus", "2"); // 订单的状态																		// 1游客
		...
		APPRestClient.post(getActivity(), port, params,
				new APPResponseHandler<OrderListItem>(OrderListItem.class,
					...
					public void onFailure(int errorCode, String errorMsg) {
						...
						/*
						 * if (errorCode == 3) { new
						 * AlertDialog.Builder(getActivity()) .setTitle("提示")
						 * .setMessage("登录超时,请退出系统重新登录！") .setPositiveButton(
						 * "确定", new DialogInterface.OnClickListener() {
						 * 
						 * @Override public void onClick( DialogInterface
						 * dialog, int which) { // 默认自动登录 AutoLogin.getInstance(
						 * getActivity()) .loginWay(); dialog.dismiss(); }
						 * }).show(); }
						 */
					}}
		...
		public void handleMessage(Message msg) {
			...
			if (msg.obj != null) {
				...
				case 1:
					...
					mSwipeLayout.setRefreshing(false);
					startId++;
					load_date_frag2.setVisibility(View.VISIBLE);
					new Handler().postDelayed(new Runnable(){   
			            public void run() {  
			                   //显示dialog
			            	loadingDialog.dismiss();			            	
			 ...
			 * else { list.clear(); adapter.notifyDataSetChanged(); }
			 */
		};
	};
	...
	public void onItemClick(AdapterView<?> parent, View view, int position,
		...
		if (position <= list.size()) {
			...
			if (list.get(position).getOrderStatus().equals("order_create")) {
				...
				intent.putExtra("orderId", orderId);
				intent.putExtra("fragment", "finish");//待接受
	...
	public void onClick(View v) {
		Intent intent = new Intent();
		switch (v.getId()) {
		case R.id.order_list_no_data_end_btn:
			// 游客模式
			if (((UUOrderActivity)getActivity()).getRole().equals("1")) {
