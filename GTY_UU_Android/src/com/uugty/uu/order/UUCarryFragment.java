...
public class UUCarryFragment extends Fragment implements
	...
	private void loadData(final int what, final boolean msg) {
		...
		params.add("orderStatus", "1"); // 订单的状态
	...
	public void onItemClick(AdapterView<?> parent, View view, int position,
		...
		if (position <= list.size()) {
			...
			if (list.get(position).getOrderStatus().equals("order_create")
				...
				intent.putExtra("orderId", orderId);
				intent.setClass(getActivity(), UUOrderDrawBackActivity.class);
			} else {
				intent.putExtra("orderId", orderId);
				intent.putExtra("fragment", "carry");//待付款
