package com.uugty.uu.shop.guide.adapter;

import android.content.Context;
import android.content.Intent;
import android.support.v4.view.PagerAdapter;
import android.text.TextUtils;
import android.view.View;
import android.view.ViewGroup;
import android.widget.ImageView;

import com.uugty.uu.entity.HomePageRecommendEntity;
import com.uugty.uu.main.RecomendActivity;
import com.uugty.uu.main.RecomendWebActivity;

import java.util.List;

/**
 * Created by lzh on 16/9/1.
 */
public class HeaderAdAdapter extends PagerAdapter {

    private Context mContext;
    private List<ImageView> ivList; // ImageView的集合
    private List<HomePageRecommendEntity.HomePageRecommend> recommendList;
    private int count = 1; // 广告的数量

    public HeaderAdAdapter(Context context, List<ImageView> ivList , List<HomePageRecommendEntity.HomePageRecommend> recommendList) {
        super();
        this.mContext = context;
        this.ivList = ivList;
        this.recommendList = recommendList;
        if(ivList != null && ivList.size() > 0){
            count = ivList.size();
        }
    }

    @Override
    public int getCount() {
        if (count == 1) {
            return 1;
        } else {
            return Integer.MAX_VALUE;
        }
    }

    @Override
    public boolean isViewFromObject(View arg0, Object arg1) {
        return arg0 == arg1;
    }

    @Override
    public void destroyItem(ViewGroup container, int position, Object object) {
    }

    @Override
    public Object instantiateItem(ViewGroup container,final int position) {
        final int newPosition = position % count;
        // 先移除在添加，更新图片在container中的位置（把iv放至container末尾）
        ImageView iv = ivList.get(newPosition);
        ivList.get(newPosition).setOnClickListener(new View.OnClickListener() {

            @Override
            public void onClick(View v) {
                Intent intent = new Intent();
                if (!TextUtils.isEmpty(recommendList.get(newPosition)
                        .getRoadlineThemeUrl())) {
                    // 跳转到webView
                    intent.putExtra("roadlineThemeTitle", recommendList
                            .get(newPosition).getRoadlineThemeTitle());
                    intent.putExtra("roadlineThemeUrl",
                            recommendList.get(newPosition)
                                    .getRoadlineThemeUrl());
                    intent.setClass(mContext, RecomendWebActivity.class);
                    mContext.startActivity(intent);
                } else {
                    // 跳转到路线
                    intent.putExtra("roadlineThemeTitle", recommendList
                            .get(newPosition).getRoadlineThemeTitle());
                    intent.putExtra("roadlineThemeId",
                            recommendList.get(newPosition)
                                    .getRoadlineThemeId());
                    intent.setClass(mContext, RecomendActivity.class);
                    mContext.startActivity(intent);
                }
            }
        });
        container.removeView(iv);
        container.addView(iv);
        return iv;
    }
}
