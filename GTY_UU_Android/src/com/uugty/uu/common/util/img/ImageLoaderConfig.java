...
public class ImageLoaderConfig {
	...
	public static DisplayImageOptions initDisplayOptions(boolean isShowDefault) {
		...
		if (isShowDefault) {
			// 默认显示的图片
			displayImageOptionsBuilder.showStubImage(R.drawable.uu_default_image_one);
			// 地址为空的默认显示图片
			displayImageOptionsBuilder.showImageForEmptyUri(R.drawable.uu_default_image_one);
			// 加载失败的显示图片
			displayImageOptionsBuilder.showImageOnFail(R.drawable.uu_default_image_one);
	...
	public static DisplayImageOptions initDisplayOptions(int targetWidth,
		...
		displayImageOptionsBuilder.displayer(new SimpleImageDisplayer(
				targetWidth));
		displayImageOptionsBuilder.displayer(new RoundedBitmapDisplayer(20));//是否设置为圆角，弧度为多少  
		return displayImageOptionsBuilder.build();
	}
	...
	public static void initImageLoader(Context context, String cacheDisc) {
		...
		//builder.memoryCacheExtraOptions(1500, 700);
