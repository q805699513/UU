...
public class AssetCopyer {
	...
	public boolean copy() throws IOException {
		...
		// 该目录存放app相关的各种文�?如cache，配置文件等)，unstall app后该目录也会随之删除
		// mContext.getExternalFilesDir()获取SDCard/Android/应用包名/files目录
		// mAppDirectory = mContext.getExternalFilesDir(null);
		mAppDirectory = new File(CacheFileUtil.rootPath);
		if (null == mAppDirectory) {
			return false;
		}

		// 读取assets/$(subDirectory)目录下的assets.lst文件，得到需要copy的文件列�?
	...
	/**
	 * 获取�?��拷贝的文件列表（记录在assets/assets.lst文件中）
	 * 
	 * @return 文件列表
	 * @throws IOException
	 */
