...
public class CalendarCard extends View {
	...
	private void init(Context context) {
		...
		todayCirclePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
		todayCirclePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
		todayCirclePaint.setStyle(Paint.Style.FILL);
		todayCirclePaint.setColor(Color.WHITE);
		...
		for (int j = 0; j < TOTAL_ROW; j++) {
			...
			for (int i = 0; i < TOTAL_COL; i++) {
				...
				if (position >= firstDayWeek
					...
					// 选中
					if (selected.contains(mShowDate.year + ","
							+ mShowDate.month + "," + i + "," + j)) {
						CustomDate date = CustomDate.modifiDayForObject(
								mShowDate, day);
						rows[j].cells[i] = new Cell(date, State.SELECTED_DAY,
								i, j);
						continue;
					}
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		mViewWidth = w;
		mViewHeight = h;
		mCellSpace = Math.min((float)mViewHeight / TOTAL_ROW, (float)mViewWidth / TOTAL_COL);
		
		if (!callBackCellSpace) {
			callBackCellSpace = true;
		}
		mTextPaint.setTextSize((int)(mCellSpace/2));
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			...
			if (Math.abs(disX) < touchSlop && Math.abs(disY) < touchSlop) {
				int col = (int) (mDownX / ((float)mViewWidth/TOTAL_COL));
				int row = (int) (mDownY / ((float)mViewHeight/TOTAL_ROW));
	...
	private void measureClickCell(int col, int row) {
		...
		if (rows[row] != null) {
			...
			if (DateUtil.compare_date(date.getToday(),
					new DateUtil().getToday())) {
				mCellClickListener.clickDate(date);
				if (selected.contains(date.year + "," + date.month + "," + col
						+ "," + row))
					selected.remove(date.year + "," + date.month + "," + col
							+ "," + row);

				else
					selected.add(date.year + "," + date.month + "," + col + ","
							+ row);
			}
	...
	class Cell {
		...
		public void drawSelf(Canvas canvas) {
			...
			case TODAY: // 今天
				 ...
				 * ((j + 0.5) * mCellSpace), mCellSpace / 3, todayCirclePaint);
				 */
				break;
			case SELECTED_DAY: // 选中的day
				mTextPaint.setColor(Color.parseColor("#fffffe"));
			...
			canvas.drawText(content,
					(float) ((i + 0.5) * ((float)mViewWidth/TOTAL_COL) - mTextPaint
							.measureText(content) / 2), (float) ((j + 0.7)
							* ((float)mViewHeight/TOTAL_ROW) - mTextPaint
	 ...
	 */
	enum State {
		TODAY, CURRENT_MONTH_DAY, PAST_MONTH_DAY, NEXT_MONTH_DAY, UNREACH_DAY, REACH_DAY, SELECTED_DAY;
	}
