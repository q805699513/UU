...
public class ScrollViewUITools {
	/**
	 * HorizontalScrollView添加阻尼效果 ScrollView效果不太好 利用父元素的Padding给ScrollView添加弹性
	 * 
	 * @param scrollView
	 * @param padding
	 */
										...
										new OnScrollChangedListener() {
											...
											public void onScrollChanged() {
												if (!inTouch
														&& scrollView != null
														&& scrollView
																.getHandler() != null) {// 如果持续滚动,移除checkStopped,停止滚动以后只执行一次检测任务
													scrollView
															.getHandler()
															.removeCallbacks(
																	checkStopped);
	...
	public static void elasticPadding(final HorizontalScrollView scrollView,
		...
		child.setPadding(padding + oldpt, child.getPaddingTop(), padding
				+ oldpb+12, child.getPaddingBottom());
		// 添加视图布局完成事件监听
		scrollView.getViewTreeObserver().addOnGlobalLayoutListener(
				new OnGlobalLayoutListener() {
					private boolean inTouch = false; // 手指是否按下状态
