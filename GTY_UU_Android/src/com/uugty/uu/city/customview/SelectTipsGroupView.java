...
public class SelectTipsGroupView extends LinearLayout {  
  
    private Context context;  
  
    public SelectTipsGroupView(Context context) {  
        super(context);  
        this.context = context;  
        setOrientation(VERTICAL);//设置方向  
    }  
  
    public SelectTipsGroupView(Context context, AttributeSet attrs) {  
        super(context, attrs);  
        this.context = context;  
        setOrientation(VERTICAL);//设置方向  
    }  
  
    public SelectTipsGroupView(Context context, AttributeSet attrs, int defStyleAttr) {  
     ...
     * @param onItemClick 
     */  
    public void initViews(String items[], final OnItemClick onItemClick,final List<String> ls) {  
        ...
        for (int i = 0; i < size; i++) {//便利items  
            ...
            for (int x = 0; x < ls.size(); x++) {	
            	if(ls.get(x).equals(items[j])){
					itemLin.setBackgroundResource(R.drawable.boder_solid_gray);
                    itemView.setTextColor(Color.parseColor("#98999a"));	
            	}
			}
            itemLin.setOnClickListener(new OnClickListener() {//给每个item设置点击事件  
                @Override  
                public void onClick(View v) {  
                    if (null != onItemClick) {  
                    	if(ls.size()>=10){
                    		CustomToast.makeText(getContext(), 0, "最对10个标签", 200).show();
                    	}else{
                    	itemLin.setBackgroundResource(R.drawable.boder_solid_gray);
                    	itemView.setTextColor(Color.parseColor("#98999a"));
                    	onItemClick.onClicks(j); 
                    	}
                    }  
                }  
            });  
    ...
    public void initViews(List<String> items, OnItemClick onItemClick) {  
        initViews((String[]) items.toArray(), onItemClick, items);  
    }  
